...
<c:if test="${not empty createList and fn:length(createList) gt 0}">
	<div class="portlet-msg-info">
		<c:forEach items="${createList}" var="create">
			<a class="portlet-menu-item"
				href="<c:out value="${create.accessUrl}" escapeXml="true"/>"><spring:message
					code="createNewList" htmlEscape="true" /> <strong>@<c:out
						value="${create.serverName}" escapeXml="true" /></strong></a>
			<br />
		</c:forEach>
	</div>
</c:if>
<form method="post" class="c"
	action="<c:out value="${actionURL}" escapeXml="true"/>">
	<span><spring:message code="search.title" htmlEscape="true" />
		: </span>
	<spring:bind path="searchForm.subscriber">
		...
		<label for="${namespace}_${status.expression}"
			class="portlet-form-field-label"><spring:message
				code="search.subscriber" htmlEscape="true" /></label>
	</spring:bind>
	<spring:bind path="searchForm.owner">
		...
		<label for="${namespace}_${status.expression}"
			class="portlet-form-field-label"><spring:message
				code="search.owner" htmlEscape="true" /></label>
	</spring:bind>
	<spring:bind path="searchForm.editor">
		...
		<label for="${namespace}_${status.expression}"
			class="portlet-form-field-label"><spring:message
				code="search.editor" htmlEscape="true" /></label>
	</spring:bind>
	<input type="submit" class="portlet-form-button"
		value="<spring:message code="search.validate" htmlEscape="true"/>" />
</form>
<c:choose>
	<c:when test="${not empty sympaList and fn:length(sympaList) gt 0}">
		<table class="data centered" border="0" cellpadding="0"
			cellspacing="0"
			summary="<spring:message code="results.summary" htmlEscape="true"/>"
			width="90%">
			<caption class="portlet-table-subheader">
				<spring:message code="results.caption"
					arguments="${fn:length(sympaList)}" htmlEscape="true" />
			</caption>
			...
			<tbody>
				<c:forEach items="${sympaList}" var="list" varStatus="varStatus">
					<tr
						<c:if test="${varStatus.index%2!=0}">class="portlet-table-alternate"</c:if>>
						<td><c:choose>
								<c:when test="${list.editor==true}">
									<a class="portlet-menu-item mailLink" href="#" target="_blank"
						...
						<td class="c centered icon_column_with_title"><insa:icon
								value="${list.subscriber}" /></td>
						<td class="c centered icon_column_with_title"><c:choose>
						...
						<td class="c centered icon_column_with_title"><insa:icon
								value="${list.editor}" /></td>
						<td class="centered icon_column_without_title"><c:choose>
								...
								<c:when test="${list.owner==true || list.subscriber==true}">
									<a class="portlet-menu-item" target="_blank"
										...
										<img src="<c:url value="/media/icons/archives.png"/>"
					 ...
					 <td class="icon_column_without_title"><c:if
								test="${list.owner==true}">
								<a class="portlet-menu-item" target="_blank"
									href="<c:out value="${list.listAdminUrl}" escapeXml="true"/>"
									title="<spring:message code="gotoListAdmin" arguments="${list.address}" htmlEscape="true"/>">
									<img src="<c:url value="/media/icons/admin_list.png"/>"
									alt="<c:out value="${iconAlt}" escapeXml="true"/>" />
								</a>
							</c:if></td> 
					</tr>
