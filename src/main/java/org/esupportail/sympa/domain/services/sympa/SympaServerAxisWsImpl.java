...
public class SympaServerAxisWsImpl extends AbstractSympaServer {
	...
	// must be session scope so the bean of type SympaServerAxisWsImpl is
	// session scope
	...
	public List<UserSympaListWithUrl> getWhich() {
		...
		try {
			 ...
			 * with Axis so we use port.which() ... whichList =
			 * SympaPort_PortType.complexWhich();
			 */
		...
		if (whichList != null) {
			for (int idx = 0; idx < whichList.length; idx++) {
				/*
				 * ListType l = whichList[idx]; UserSympaListWithUrl item = new
				 * UserSympaListWithUrl(); item.setEditor(l.getIsEditor());
	...
	private SympaPort_PortType getPort() throws MalformedURLException, ServiceException, RemoteException {
		...
		((SympaSOAPLocator) locator).setMaintainSession(true); // mandatory for
																// cookie after
																// login
		...
		case cas:
			...
			((SOAPStub) port)._setProperty(HTTPConstants.HEADER_COOKIE, "sympa_session=" + tmp);
		...
		case password:
			...
			((SOAPStub) port)._setProperty(HTTPConstants.HEADER_COOKIE, "sympa_session=" + tmp2);
			if (logger.isDebugEnabled()) {
